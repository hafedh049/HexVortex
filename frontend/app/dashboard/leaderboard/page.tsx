...
import { Trophy, Shield, Medal, Award } from "lucide-react"
...
export default function LeaderboardPage() {
  return (
    <div className="relative min-h-screen overflow-hidden">
      ...
      <div className="relative z-10 container mx-auto px-4 py-16 border border-[hsl(var(--accent)/0.3)] rounded-xl">
                        ...
                        <div className="flex items-center gap-4">
                          <Avatar className={index < 3 ? "h-10 w-10" : "h-8 w-8"}>
                            {team.avatar ? (
                              ...
                              >
                                {index === 0 ? (
                                  <Trophy className={index < 3 ? "h-6 w-6" : "h-4 w-4"} />
                                ) : index === 1 ? (
                                  <Medal className={index < 3 ? "h-6 w-6" : "h-4 w-4"} />
                                ) : index === 2 ? (
                                  <Award className={index < 3 ? "h-6 w-6" : "h-4 w-4"} />
                                ) : (
                  ...
                  {[
                    ...
                    <div key={index} className="p-4 rounded-lg bg-black/50 border border-purple-900/20">
                      <div className="flex justify-between items-center mb-3">
                        ...
                        <Badge
                          variant="outline"
                          className="bg-[hsl(var(--accent))] text-white border-[hsl(var(--accent))]"
                        >
                          ...
                          {category.teams.map((team, idx) => (
                            ...
                            >
                              <div className="flex items-center gap-2">
                                <div
                                  className={`w-5 h-5 rotate-45 flex items-center justify-center p-0.5 ${idx === 0 ? "bg-yellow-900/50 text-yellow-400" : idx === 1 ? "bg-gray-700/50 text-gray-300" : "bg-amber-900/50 text-amber-600"}`}
                                >
                                  <span className="-rotate-45">{idx + 1}</span>
                                </div>
                                <span className="text-white">{team.name}</span>
                              </div>
                              <span className="font-medium text-[hsl(var(--accent))]">{team.score} pts</span>
                            </div>
                          ))}
                        </div>
                      </CardContent>
